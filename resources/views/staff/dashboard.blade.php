...
@section('content')
    @php
        ...
        $stats = [
            ['icon' => 'people', 'value' => '1,234', 'label' => 'Parishioners', 'change' => 'â†‘ 12 this month'],
            ['icon' => 'requests', 'value' => '47', 'label' => 'Pending Requests', 'change' => 'â†‘ 3 since yesterday'],
            ['icon' => 'calendar', 'value' => '28', 'label' => 'Upcoming Events', 'change' => 'â†“ 4 next week'],
            ['icon' => 'donations', 'value' => 'â‚±45,230', 'label' => 'Donations', 'change' => 'â†‘ â‚±2,800 this week'],
        ];
    @endphp
        ...
        @foreach($stats as $stat)
            <article class="stat">
                <div class="icon">
                    @switch($stat['icon'])
                        @case('people')â™™@break
                        @case('requests')â–¤@break
                        @case('calendar')â–£@break
                        @defaultâœ£
                    @endswitch
                </div>
    ...
    <div class="grid">
        <section class="panel">
            ...
            @foreach([['â™™','Baptism Request â€” Maria Santos','Today at 9:30 AM'],['â™¡','Mass Intention â€” Roberto Cruz','Yesterday'],['â™¢','Wedding Request â€” Ana & Miguel','August 20']] as $item)
                <div class="request">
                    <i>{{ $item[0] }}</i>
                    <div>
                        <b>{{ $item[1] }}</b>
        ...
        <section class="panel">
            ...
            <div class="quick">
                <button><span>ï¼‹</span>Add event</button>
                <button><span>âœ¦</span>Post notice</button>
                <button><span>â–£</span>Mass schedule</button>
                <button><span>â™™</span>Add member</button>
            </div>
        </section>
    </div>
@endsection
